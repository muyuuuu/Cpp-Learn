...
可以『无参调用』的构造函数，不是说构造函数没有参数。所以构造函数可以是无参数列表，也可以是所有参数都有默认参数值的构造函数。
...
类中的对象成员没有被显式初始化，调自己类的无参构造函数，对象会构造成功。没有无参构造函数，会报错。

# 成员的初始化次序

初始化的执行次序：就地初始化，构造函数初始化列表，构造函数体内为成员赋值。所以起作用的顺序是：构造函数体内为成员赋值，构造函数初始化列表，就地初始化。

# 字符串和数组类

C语言中，字符串存到字符数组，以空字符结尾。

C语言风格的数组：
1. 可能退化为指针，`sizeof(a)/sizeof(a[0])`失效
2. 不知道自己的大小
3. 数组元素无法直接复制, `a = b`
4. 不能自动推导类型，`auto a[] = {1, 2, 3}`

C++风格数组：
1. 是一个容器类，有迭代器
2. 是模板类，容纳任何类型的数据，但数组不能扩大也不能缩小，数据类型相同
3. 对类模板的参数进行推导，推导出类型和数量
4. 可以直接赋值
5. 知道自己的大小 `size()`
6. 和另一个数组交换内容 `swap()`
7. 用指定内容填充自己 `fill()`
